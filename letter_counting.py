...
def _parse_args():
    ...
    parser.add_argument(
        "--task", type=str, default="BEFORE", help="task to run (BEFORE or BEFOREAFTER)"
    )
    parser.add_argument(
        "--train",
        type=str,
        default="data/lettercounting-train.txt",
        help="path to train examples",
    )
    parser.add_argument(
        "--dev",
        type=str,
        default="data/lettercounting-dev.txt",
        help="path to dev examples",
    )
    parser.add_argument(
        "--output_bundle_path",
        type=str,
        default="classifier-output.json",
...
def get_letter_count_output(input: str, count_only_previous: bool = True) -> np.array:
    ...
    for i in range(0, len(input)):
        if count_only_previous:
            ...
            output[i] = min(
                2, len([c for c in input if c == input[i]]) - 1
            )  # count all *other* instances of input[i]
    return output
...
if __name__ == "__main__":
    ...
    train_bundles = [
        LetterCountingExample(
            l, get_letter_count_output(l, count_only_previous), vocab_index
        )
        for l in train_exs
    ]

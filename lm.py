...
def _parse_args():
    ...
    parser.add_argument(
        "--model", type=str, default="UNIFORM", help="model to run (UNIFORM or NEURAL)"
    )
    parser.add_argument(
        "--train_path",
        type=str,
        default="data/text8-100k.txt",
        help="path to train set (you should not need to modify)",
    )
    parser.add_argument(
        "--dev_path",
        type=str,
        default="data/text8-dev.txt",
    ...
    parser.add_argument(
        ...
        type=str,
        default="output.json",
        help="path to write the results json to (you should not need to modify)",
    )
    ...
    for context in contexts:
        for next_seq in next_seqs:
            ...
            if log_prob > 0.0:
                sane = False
                print(
                    "ERROR: sanity checks failed, LM log probability %f is invalid"
                    % (log_prob)
                )
            ...
            for i in range(0, len(next_seq)):
                ...
                next_char_log_probs = lm.get_next_char_log_probs(
                    context + next_seq[0:i]
                )
                # print(repr(next_char_log_probs))
                log_prob_from_single_probs += next_char_log_probs[
                    vocab_index.index_of(next_seq[i])
                ]
            if abs(log_prob_from_single_probs - log_prob) > 1e-3:
                sane = False
                print(
                    "ERROR: sanity checks failed, LM prob from sequence and single characters disagree: %f %f"
                    % (log_prob, log_prob_from_single_probs)
                )
    return sane
...
def normalization_test(lm, vocab_index):
    ...
    for context in contexts:
        ...
        if total_prob_single < 0.99 or total_prob_single > 1.01:
            normalizes = False
            print(
                "Probabilities sum to %f not 1.0 for context %s"
                % (total_prob_single, context)
            )
        total_prob_seq = 0.0
        for char_idx in range(0, len(vocab_index)):
            total_prob_seq += np.exp(
                lm.get_log_prob_sequence(vocab_index.get_object(char_idx), context)
            )
        if total_prob_seq < 0.99 or total_prob_seq > 1.01:
            normalizes = False
            print(
                "Probabilities sum to %f not 1.0 for context %s"
                % (total_prob_seq, context)
            )
    return normalizes
...
def perplexity_range_check(perplexity):
    if perplexity < 3.5:
        print(
            "ERROR: checks failed, the perplexity is too low. Please make sure you are using causal mask and make sure you are scoring the entire next_chars (instead of a single chunk) in get_log_prob_sequence"
        )
        return False
    return True
...
if __name__ == "__main__":
    ...
    if args.model == "NEURAL":
        ...
        raise Exception(
            "Pass in either UNIFORM or NEURAL to run the appropriate system"
        )

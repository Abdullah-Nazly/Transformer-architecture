...
#### Bidirectional vs Backward-Only Attention

**Self-attention is inherently bidirectional:**

- By default, each position can attend to **all positions** (past, present, and future)
- This allows the model to use full context from the entire sequence

**Causal masking restricts to backward-only:**

- When `use_causal_mask=True`, attention becomes **backward-only (causal)**
- Each position can only attend to **past and present positions** (not future)
- This is needed for autoregressive tasks like language modeling

**Summary:**

- **Without causal mask** (`use_causal_mask=False`): **Bidirectional attention** - can see all positions
- **With causal mask** (`use_causal_mask=True`): **Backward-only attention** - can only see past/present

**In your code:**

- Letter-counting task: Default is bidirectional (no causal mask)
- Language modeling: Uses backward-only (causal mask enabled)
